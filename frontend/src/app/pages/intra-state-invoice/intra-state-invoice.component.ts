...
@Component({
  selector: 'app-intra-state-invoice',
  templateUrl: './intra-state-invoice.component.html',
  styleUrls: ['./intra-state-invoice.component.css'],
})
export class IntraStateInvoiceComponent implements OnInit {
  intraForm: FormGroup;
  submitted = false;
  ...
  onSubmit(val) {
    ...
    if (this.intraForm.invalid) {
      ...
      this.authService
        .post('invoice', this.intraForm.value)
        .subscribe((res) => {
